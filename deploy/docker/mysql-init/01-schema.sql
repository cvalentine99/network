...
SELECT `d`.`id` AS `id`,`d`.`display_name` AS `display_name`,`d`.`default_name` AS `default_name`,`d`.`custom_name` AS `custom_name`,`d`.`macaddr` AS `macaddr`,`d`.`ipaddr4` AS `ipaddr4`,`d`.`ipaddr6` AS `ipaddr6`,`d`.`device_class` AS `device_class`,`d`.`vendor` AS `vendor`,`d`.`role` AS `role`,`d`.`analysis` AS `analysis`,`d`.`analysis_level` AS `analysis_level`,`d`.`on_watchlist` AS `on_watchlist`,`d`.`critical` AS `critical`,`d`.`dns_name` AS `dns_name`,`d`.`dhcp_name` AS `dhcp_name`,`d`.`cloud_instance_name` AS `cloud_instance_name`,`d`.`cloud_account` AS `cloud_account`,FROM_UNIXTIME(`d`.`discover_time`/1000) AS `discover_datetime`,FROM_UNIXTIME(`d`.`last_seen_time`/1000) AS `last_seen_datetime`,`d`.`polled_at` AS `polled_at` FROM `FAgcVHDa53BkVwvRfvbPzh`.`dim_device` AS `d` WHERE `d`.`last_seen_time` IS NOT NULL;

-- View: v_alert_summary
CREATE OR REPLACE VIEW `v_alert_summary` AS
SELECT `a`.`id` AS `alert_id`,`a`.`name` AS `name`,`a`.`type` AS `type`,`a`.`severity` AS `severity`,`a`.`disabled` AS `disabled`,`a`.`stat_name` AS `stat_name`,`a`.`field_name` AS `field_name`,`a`.`operator` AS `operator`,`a`.`operand` AS `operand`,`a`.`units` AS `units`,`a`.`interval_length` AS `interval_length`,`a`.`refire_interval` AS `refire_interval`,`a`.`apply_all` AS `apply_all`,`a`.`polled_at` AS `polled_at` FROM `FAgcVHDa53BkVwvRfvbPzh`.`dim_alert` AS `a`;

-- View: v_appliance_overview
CREATE OR REPLACE VIEW `v_appliance_overview` AS
SELECT `a`.`id` AS `id`,`a`.`hostname` AS `hostname`,`a`.`display_name` AS `display_name`,`a`.`platform` AS `platform`,`a`.`firmware_version` AS `firmware_version`,`a`.`license_status` AS `license_status`,`a`.`connection_type` AS `connection_type`,`a`.`manages_local` AS `manages_local`,`a`.`managed_by_local` AS `managed_by_local`,`a`.`data_access` AS `data_access`,`a`.`total_capacity` AS `total_capacity`,`a`.`advanced_analysis_capacity` AS `advanced_analysis_capacity`,`a`.`nickname` AS `nickname`,`a`.`polled_at` AS `polled_at` FROM `FAgcVHDa53BkVwvRfvbPzh`.`dim_appliance` AS `a`;

-- View: v_device_current_ips
CREATE OR REPLACE VIEW `v_device_current_ips` AS
SELECT `d`.`id` AS `device_id`,`d`.`display_name` AS `display_name`,`d`.`macaddr` AS `macaddr`,`s`.`ipaddr` AS `ipaddr`,FROM_UNIXTIME(`s`.`last_observation_time`/1000) AS `last_observed_datetime` FROM `FAgcVHDa53BkVwvRfvbPzh`.`dim_device` AS `d` JOIN `FAgcVHDa53BkVwvRfvbPzh`.`snap_device_ipaddr` AS `s` ON `s`.`device_id`=`d`.`id` AND `s`.`is_current`=TRUE;

-- View: v_device_group_membership
CREATE OR REPLACE VIEW `v_device_group_membership` AS
SELECT `dg`.`id` AS `group_id`,`dg`.`name` AS `group_name`,`dg`.`built_in` AS `built_in`,`dg`.`dynamic` AS `dynamic`,`d`.`id` AS `device_id`,`d`.`display_name` AS `device_name`,`d`.`device_class` AS `device_class`,`d`.`role` AS `role` FROM (`FAgcVHDa53BkVwvRfvbPzh`.`dim_device_group` AS `dg` JOIN `FAgcVHDa53BkVwvRfvbPzh`.`bridge_device_device_group` AS `b` ON `b`.`device_group_id`=`dg`.`id`) JOIN `FAgcVHDa53BkVwvRfvbPzh`.`dim_device` AS `d` ON `d`.`id`=`b`.`device_id`;

-- View: v_metric_responses
CREATE OR REPLACE VIEW `v_metric_responses` AS
SELECT `mr`.`id` AS `response_id`,`mr`.`cycle` AS `cycle`,`mr`.`metric_category` AS `metric_category`,`mr`.`object_type` AS `object_type`,FROM_UNIXTIME(`mr`.`from_time`/1000) AS `from_datetime`,FROM_UNIXTIME(`mr`.`until_time`/1000) AS `until_datetime`,`ms`.`oid` AS `oid`,`ms`.`duration` AS `duration`,`ms`.`values_json` AS `values_json`,FROM_UNIXTIME(`ms`.`stat_time`/1000) AS `stat_datetime`,`mr`.`polled_at` AS `polled_at` FROM `FAgcVHDa53BkVwvRfvbPzh`.`fact_metric_response` AS `mr` JOIN `FAgcVHDa53BkVwvRfvbPzh`.`fact_metric_stat` AS `ms` ON `ms`.`metric_response_id`=`mr`.`id`;

-- View: v_record_searches
CREATE OR REPLACE VIEW `v_record_searches` AS
SELECT `rs`.`id` AS `search_id`,FROM_UNIXTIME(`rs`.`from_time`/1000) AS `from_datetime`,FROM_UNIXTIME(`rs`.`until_time`/1000) AS `until_datetime`,`rs`.`total` AS `total`,`rs`.`terminated_early` AS `terminated_early`,`rs`.`lookback_truncated` AS `lookback_truncated`,`rs`.`lookback_exceeded` AS `lookback_exceeded`,`r`.`record_id` AS `record_id`,`r`.`record_type` AS `record_type`,`r`.`appliance_uuid` AS `appliance_uuid`,`r`.`source_json` AS `source_json`,`rs`.`polled_at` AS `polled_at` FROM `FAgcVHDa53BkVwvRfvbPzh`.`fact_record_search` AS `rs` JOIN `FAgcVHDa53BkVwvRfvbPzh`.`fact_record` AS `r` ON `r`.`search_id`=`rs`.`id`;

-- View: vw_alert_assignments_current
CREATE OR REPLACE VIEW `vw_alert_assignments_current` AS
SELECT `a`.`id` AS `alert_id`,`a`.`name` AS `name`,`a`.`type` AS `type`,`a`.`severity` AS `severity`,`a`.`disabled` AS `disabled`,`a`.`stat_name` AS `stat_name`,`a`.`field_name` AS `field_name`,`a`.`operator` AS `operator`,`a`.`operand` AS `operand`,`a`.`units` AS `units`,`a`.`interval_length` AS `interval_length`,`a`.`refire_interval` AS `refire_interval`,`a`.`apply_all` AS `apply_all`,(SELECT COUNT(1) AS `COUNT(*)` FROM `FAgcVHDa53BkVwvRfvbPzh`.`bridge_alert_device` AS `bd` WHERE `bd`.`alert_id`=`a`.`id`) AS `device_count`,(SELECT COUNT(1) AS `COUNT(*)` FROM `FAgcVHDa53BkVwvRfvbPzh`.`bridge_alert_device_group` AS `bg` WHERE `bg`.`alert_id`=`a`.`id`) AS `group_count`,(SELECT COUNT(1) AS `COUNT(*)` FROM `FAgcVHDa53BkVwvRfvbPzh`.`bridge_alert_application` AS `ba` WHERE `ba`.`alert_id`=`a`.`id`) AS `app_count`,(SELECT COUNT(1) AS `COUNT(*)` FROM `FAgcVHDa53BkVwvRfvbPzh`.`bridge_alert_network` AS `bn` WHERE `bn`.`alert_id`=`a`.`id`) AS `network_count`,`a`.`raw_id` AS `raw_id`,`a`.`polled_at` AS `polled_at` FROM `FAgcVHDa53BkVwvRfvbPzh`.`dim_alert` AS `a`;

-- View: vw_device_dns_current
CREATE OR REPLACE VIEW `vw_device_dns_current` AS
SELECT `s`.`device_id` AS `device_id`,`d`.`display_name` AS `display_name`,`s`.`dns_name` AS `dns_name`,`s`.`raw_id` AS `raw_id`,`s`.`polled_at` AS `polled_at` FROM `FAgcVHDa53BkVwvRfvbPzh`.`snap_device_dnsname` AS `s` JOIN `FAgcVHDa53BkVwvRfvbPzh`.`dim_device` AS `d` ON `d`.`id`=`s`.`device_id` WHERE `s`.`is_current`=TRUE;

-- View: vw_device_inventory_current
CREATE OR REPLACE VIEW `vw_device_inventory_current` AS
SELECT `d`.`id` AS `device_id`,`d`.`display_name` AS `display_name`,`d`.`default_name` AS `default_name`,`d`.`custom_name` AS `custom_name`,`d`.`macaddr` AS `macaddr`,`d`.`ipaddr4` AS `ipaddr4`,`d`.`ipaddr6` AS `ipaddr6`,`d`.`device_class` AS `device_class`,`d`.`vendor` AS `vendor`,`d`.`role` AS `role`,`d`.`auto_role` AS `auto_role`,`d`.`analysis` AS `analysis`,`d`.`analysis_level` AS `analysis_level`,`d`.`is_l3` AS `is_l3`,`d`.`vlanid` AS `vlanid`,`d`.`on_watchlist` AS `on_watchlist`,`d`.`critical` AS `critical`,`d`.`dns_name` AS `dns_name`,`d`.`dhcp_name` AS `dhcp_name`,`d`.`netbios_name` AS `netbios_name`,`d`.`cdp_name` AS `cdp_name`,FROM_UNIXTIME(`d`.`discover_time`/1000) AS `discover_datetime`,FROM_UNIXTIME(`d`.`last_seen_time`/1000) AS `last_seen_datetime`,FROM_UNIXTIME(`d`.`mod_time`/1000) AS `mod_datetime`,`d`.`raw_id` AS `raw_id`,`d`.`polled_at` AS `polled_at` FROM `FAgcVHDa53BkVwvRfvbPzh`.`dim_device` AS `d`;

-- View: vw_device_ip_current
CREATE OR REPLACE VIEW `vw_device_ip_current` AS
SELECT `s`.`device_id` AS `device_id`,`d`.`display_name` AS `display_name`,`s`.`ipaddr` AS `ipaddr`,FROM_UNIXTIME(`s`.`last_observation_time`/1000) AS `last_observed_datetime`,`s`.`raw_id` AS `raw_id`,`s`.`polled_at` AS `polled_at` FROM `FAgcVHDa53BkVwvRfvbPzh`.`snap_device_ipaddr` AS `s` JOIN `FAgcVHDa53BkVwvRfvbPzh`.`dim_device` AS `d` ON `d`.`id`=`s`.`device_id` WHERE `s`.`is_current`=TRUE;

-- View: vw_metric_latest_by_object
CREATE OR REPLACE VIEW `vw_metric_latest_by_object` AS
SELECT `mr`.`metric_category` AS `metric_category`,`mr`.`object_type` AS `object_type`,`ms`.`oid` AS `oid`,`ms`.`stat_time` AS `stat_time`,`ms`.`duration` AS `duration`,`ms`.`values_json` AS `values_json`,FROM_UNIXTIME(`ms`.`stat_time`/1000) AS `stat_datetime`,`mr`.`raw_id` AS `raw_id`,`mr`.`polled_at` AS `polled_at` FROM `FAgcVHDa53BkVwvRfvbPzh`.`fact_metric_stat` AS `ms` JOIN `FAgcVHDa53BkVwvRfvbPzh`.`fact_metric_response` AS `mr` ON `mr`.`id`=`ms`.`metric_response_id` WHERE `ms`.`stat_time`=(SELECT MAX(`ms2`.`stat_time`) AS `MAX(ms2.stat_time)` FROM `FAgcVHDa53BkVwvRfvbPzh`.`fact_metric_stat` AS `ms2` JOIN `FAgcVHDa53BkVwvRfvbPzh`.`fact_metric_response` AS `mr2` ON `mr2`.`id`=`ms2`.`metric_response_id` WHERE `mr2`.`metric_category`=`mr`.`metric_category` AND `ms2`.`oid`=`ms`.`oid`);

-- View: vw_topology_edges_by_snapshot
CREATE OR REPLACE VIEW `vw_topology_edges_by_snapshot` AS
SELECT `e`.`id` AS `edge_id`,`e`.`topology_id` AS `topology_id`,`t`.`activity_map_id` AS `activity_map_id`,`e`.`source_node_id` AS `source_node_id`,`src`.`display_name` AS `source_display_name`,`src`.`ipaddr` AS `source_ipaddr`,`e`.`target_node_id` AS `target_node_id`,`tgt`.`display_name` AS `target_display_name`,`tgt`.`ipaddr` AS `target_ipaddr`,`e`.`protocol` AS `protocol`,`e`.`weight` AS `weight`,`e`.`bytes_in` AS `bytes_in`,`e`.`bytes_out` AS `bytes_out`,`e`.`edge_data` AS `edge_data`,`t`.`raw_id` AS `raw_id`,`t`.`polled_at` AS `polled_at` FROM ((`FAgcVHDa53BkVwvRfvbPzh`.`snap_topology_edge` AS `e` JOIN `FAgcVHDa53BkVwvRfvbPzh`.`snap_topology` AS `t` ON `t`.`id`=`e`.`topology_id`) LEFT JOIN `FAgcVHDa53BkVwvRfvbPzh`.`snap_topology_node` AS `src` ON `src`.`id`=`e`.`source_node_id`) LEFT JOIN `FAgcVHDa53BkVwvRfvbPzh`.`snap_topology_node` AS `tgt` ON `tgt`.`id`=`e`.`target_node_id`;

-- View: vw_topology_latest_edges
CREATE OR REPLACE VIEW `vw_topology_latest_edges` AS
SELECT `e`.`id` AS `edge_id`,`e`.`topology_id` AS `topology_id`,`t`.`activity_map_id` AS `activity_map_id`,`e`.`source_node_id` AS `source_node_id`,`src`.`display_name` AS `source_display_name`,`src`.`ipaddr` AS `source_ipaddr`,`e`.`target_node_id` AS `target_node_id`,`tgt`.`display_name` AS `target_display_name`,`tgt`.`ipaddr` AS `target_ipaddr`,`e`.`protocol` AS `protocol`,`e`.`weight` AS `weight`,`e`.`bytes_in` AS `bytes_in`,`e`.`bytes_out` AS `bytes_out`,`e`.`edge_data` AS `edge_data`,`t`.`raw_id` AS `raw_id`,`t`.`polled_at` AS `polled_at` FROM ((`FAgcVHDa53BkVwvRfvbPzh`.`snap_topology_edge` AS `e` JOIN `FAgcVHDa53BkVwvRfvbPzh`.`snap_topology` AS `t` ON `t`.`id`=`e`.`topology_id`) LEFT JOIN `FAgcVHDa53BkVwvRfvbPzh`.`snap_topology_node` AS `src` ON `src`.`id`=`e`.`source_node_id`) LEFT JOIN `FAgcVHDa53BkVwvRfvbPzh`.`snap_topology_node` AS `tgt` ON `tgt`.`id`=`e`.`target_node_id` WHERE `t`.`polled_at`=(SELECT MAX(`polled_at`) AS `MAX(polled_at)` FROM `FAgcVHDa53BkVwvRfvbPzh`.`snap_topology`);

-- View: vw_topology_latest_nodes
CREATE OR REPLACE VIEW `vw_topology_latest_nodes` AS
SELECT `n`.`id` AS `node_id`,`n`.`topology_id` AS `topology_id`,`t`.`activity_map_id` AS `activity_map_id`,`n`.`object_type` AS `object_type`,`n`.`object_id` AS `object_id`,`n`.`ipaddr` AS `ipaddr`,`n`.`display_name` AS `display_name`,`n`.`role` AS `role`,`n`.`is_external` AS `is_external`,`n`.`weight` AS `weight`,`n`.`node_data` AS `node_data`,`t`.`raw_id` AS `raw_id`,`t`.`polled_at` AS `polled_at` FROM `FAgcVHDa53BkVwvRfvbPzh`.`snap_topology_node` AS `n` JOIN `FAgcVHDa53BkVwvRfvbPzh`.`snap_topology` AS `t` ON `t`.`id`=`n`.`topology_id` WHERE `t`.`polled_at`=(SELECT MAX(`polled_at`) AS `MAX(polled_at)` FROM `FAgcVHDa53BkVwvRfvbPzh`.`snap_topology`);
